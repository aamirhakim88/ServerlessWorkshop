...
def print_stats(traces):
    ...
    print(worker_function_arn)
    durations = []
    for _segment in traces[u'Traces'][0]['Segments']:
        segment = json.loads(_segment['Document'])
        subsegments = segment.get('subsegments', [])
        for subsegment in subsegments:
            function_arn = subsegment.get('aws', {}).get('function_arn')
            if subsegment['name'] == 'Initialization' and function_arn == worker_function_arn:
    ...
    if len(durations) is 0:
        ...
        print("Average Initialization Time = {}".format(sum(durations)/len(durations)))
